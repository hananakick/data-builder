...
`data-builder`는 스키마 빌더와 타입 레지스트리를 제공하는 TypeScript 라이브러리입니다. 주요 목적은 다양한 데이터 타입을 구조화되고 타입 안전하게 정의하고 관리하는 데 있습니다. 이 라이브러리는 다음 주요 기능과 이점을 제공합니다:

### 주요 기능

1. **스키마 빌더**: 다음 데이터 타입의 스키마를 생성할 수 있습니다:
   - 기본 타입(문자열, 숫자, 불리언)
   - 배열(동질 배열 및 튜플)
   - 중첩된 속성을 가진 객체
   - 사용자 정의 타입 및 검증 로직
...
3. **값 노드 팩토리**: 타입 정보와 실제 값을 캡슐화하는 강하게 타입화된 값 노드를 생성하며, 내장된 검증을 제공합니다.
...
이 라이브러리를 사용할 때는 다음을 수행해야 할 때입니다:
- 복잡한 데이터 구조를 타입 안전하게 검증
- 애플리케이션 전체에서 타입 정의 관리 및 재사용
- 처리 전에 데이터가 특정 스키마에 맞는지 확인
- 특정 데이터 형식에 대한 사용자 정의 검증 로직 생성
...
중첩된 객체 및 배열이 있는 더 복잡한 타입을 생성합니다:
...
이 라이브러리는 타입 정보와 실제 값을 캡슐화하는 강하게 타입화된 값 노드를 생성하는 팩토리를 제공하며, 내장된 검증을 제공합니다.
...
**목적**: 이 예제는 다양한 타입(사용자 정의 타입 포함)의 값 노드를 생성하는 방법을 보여주고 검증이 어떻게 작동하는지 보여줍니다.

### 3. 타입 호환성 확인

이 라이브러리는 스키마를 기반으로 다양한 타입 간의 호환성을 확인할 수 있어 타입 검증 및 변환에 유용합니다.
...
### 4. 스키마 빌더를 사용하여 복잡한 타입 사용

스키마 빌더는 배열, 객체 및 사용자 정의 타입을 포함한 복잡한 데이터 구조를 정의할 수 있게 합니다.
...
`SchemaBuilder` 객체는 다양한 유형의 스키마 정의 생성을 위한 메서드를 제공합니다.
...
- `items` (Schema[]): 스키마 정의 배열.
  ...
  - 튜플(예: `[string, number]`)의 경우 각 요소의 스키마가 있는 배열을 전달합니다: `[SchemaBuilder.string(), SchemaBuilder.number()]`.
- `options` (객체, 선택 사항):
...
- `options` (객체, 선택 사항):
  - `validator` ((value: any) => boolean, 선택 사항): 이 사용자 정의 타입의 값을 검증하는 함수. 유효한 경우 true, 그렇지 않은 경우 false를 반환합니다.
...
// (다른 곳에서 자체 스키마 및/또는 검증기를 가진 'UUID' 타입이 등록되어 있다고 가정)

// 기본 스키마(의미 부여 또는 별칭 추가에 유용)를 가진 사용자 정의 타입
...
**반환:**
- `TypeDefinition | undefined`: 찾은 경우 타입 정의, 그렇지 않은 경우 undefined.
...
`ValueNodeFactory` 클래스는 타입 검증이 있는 값 노드 생성을 위한 메서드를 제공합니다.
...
**반환:**
- `ValueNode<T, U>`: 지정된 타입 및 값이 있는 값 노드.
...
**반환:**
- `{ success: true; node: ValueNode<T, U> }`: 성공한 경우 값 노드.
- `{ success: false; error: string }`: 실패한 경우 오류 메시지.

### SchemaValidator

`SchemaValidator` 클래스는 값이 스키마에 맞는지 검증하는 메서드를 제공합니다.
...
`TypeCompatibility` 클래스는 타입 간의 호환성을 확인하는 메서드를 제공합니다.
...
**반환:**
- `CompatibilityResult`: 호환성 결과 및 이유.
...
`defineType` 함수는 사용자 정의 타입을 정의하고 등록하는 데 사용됩니다.

**매개변수:**
- `typeName` (문자열): 타입 이름.
- `schema` (Schema): 타입 스키마.
- `description` (문자열, 선택 사항): 타입 설명.

**반환:**
- `TypeDefinition`: 정의된 타입.

**예제:**
```typescript
import { defineType } from 'data-builder';
import { SchemaBuilder } from 'data-builder';

// 사용자 정의 이메일 타입 정의 및 등록
const EmailType = defineType(
  'email',
  SchemaBuilder.custom('email', {
    validator: (value) => typeof value === 'string' && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value)
